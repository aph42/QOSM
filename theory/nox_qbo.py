...
def run_nox_qbo(analytical_base = False):
# {{{
   # Get some basic state profiles from data files generated by Alison
   run = 'ref'
   dsr = open_rce_file(run)
...
# {{{
   ...
   fNOx   = pyg.Var((st_pres, ), name = 'fN2O',   values = np.absolute(St.wp) * init_dNOx_0_dz(St.zs))
   ...
   axe = pyg.showlines([to_days(v) for v in [eNOx, eAdv, eTrs]], labels = [r'$1/\epsilon_{NO_x}$', r'$D / w_0$', r'$1/\omega$'], size = size)
   axe.setp(ylim = ylims, title = 'Timescales', xlabel = r'days', xscale = 'log')
   axe.setp_xaxis(major_formatter = plt.LogFormatter(), major_locator = plt.LogLocator(subs =  [1., 3]))

   axf = pyg.showlines([to_ppbvperday(v) for v in [fN2O, fNOx]], labels = [r'N$_2$O oxidation', 'Advection of NO$_x$'], size = size)
   axf.setp(ylim = ylims, title = 'Sources', xlabel = r'ppbv day$^{-1}$', xscale = 'log', xlim = [3e-5, 8e-2])
   axf.setp_xaxis(major_formatter = plt.LogFormatter(), major_locator = plt.LogLocator(), minor_locator = plt.LogLocator(base=10.))
...
# {{{
   ...
   NOx_0  = pyg.Var((st_pres, ), name = 'NOx_0', values = init_NOx_0(St.zs))
   ...
   axn.setp(ylim = ylims, title = r'$\partial_z$ N$_2$O', xlabel = 'vmr m$^{-1}$', xlim = [-5e-11, 5e-11])

   axx = pyg.showlines([ds0.NOx.deriv('pres', dx=st_zs), dNOxdz], labels = ['calculated', 'fit'], fmts = ['C0', 'k--'], size = size)
...
# {{{
   ...
   St, ds = run_nox_qbo(analytical_base = False)
   ...
   def make_pair(ds, var, unit, c, dsref = None):
      ...
      axp.setp(title = f'{var}: Phase', xlabel = '')
      set_rad_axis(axp)
   ...
   axs.append(make_pair(dsx, r'NO$_x$', 'ppbv', 'C0', dsref = dn(pres = (110, 4.8))))
